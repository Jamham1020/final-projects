...
select maker from (
     select maker, count(*) as count from pc join product on pc.model=product.model 
     where speed >= 2.80 group by maker
     union all
     select maker, count(*) as count from laptop join product on laptop.model=product.model 
     where speed >= 2.80 group by maker
) as fc
group by maker
having sum(count) >= 2
...
with t1 as (select model, speed from pc union select model, speed from laptop )
select distinct maker from product join t1 on product.model=t1.model 
where t1.speed = (select max(speed) from t1);
